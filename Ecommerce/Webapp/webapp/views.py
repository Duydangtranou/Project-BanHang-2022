...
def product_list(request):
    if request.method == 'GET':
        products = Product.objects.all().order_by('create_on').reverse()
...
@csrf_exempt
def productImg_productId(request,productId):
    try:
        productImg = ProductImg.objects.get(productId=productId)
    except:
        return HttpResponse(status=404)
...
def cart_item_by_cart(request,cartId):
    try:
        cartItem = Product.objects.filter(cartId=cartId)
    except:
        return HttpResponse(status=404)
...
@csrf_exempt
def cart_item_check_sameproduct(request,cartId,productId):
    try:
        cartItem = Product.objects.filter(cartId=cartId).filter(productId=productId)
    except:
        return HttpResponse(status=404)
...
def get_store(request,userId):
    try:
        store = Product.objects.filter(userId=userId)
    except:
        return HttpResponse(status=404)
...
@csrf_exempt
def filter_price_and_brand(request,minprice,maxprice,brand):
    try:
        products = Product.objects.filter(price__range=(minprice, maxprice)).filter(brand=brand)
    except:
        return HttpResponse(status=404)
...
@csrf_exempt
def filter_rating_and_brand(request, rating, brand):
    try:
        products = Product.objects.filter(rating_gte=rating).filter(brand=brand)
    except:
        return HttpResponse(status=404)
...
@csrf_exempt
def filter_condition_and_brand(request, condition, brand):
    try:
        products = Product.objects.filter(condition=condition).filter(brand=brand)
    except:
        return HttpResponse(status=404)
...
@csrf_exempt
def filter_all(request,minprice, maxprice, rating, condition, brand):
    try:
        products = Product.objects.filter(price__range=(minprice, maxprice)).filter(rating_gte=rating).filter(condition=condition).filter(brand=brand)
    except:
        return HttpResponse(status=404)
...
@csrf_exempt
def get_cart_item_by_cartId(request,cartId):
    try:
